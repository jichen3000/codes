...
;           (p ,tt)))))
(define (review-post req post-title)
  (printf "review-post \n")
  (response/full
   200 #"Okay"
   ...
   (list (string->bytes/utf-8 
          (fast-include post-title)))))

(define (fast-include post-title)
  (let ([clients (list (cons "Young" "Brigham") (cons "Smith" "Joseph"))])
    (include-template "public/viewpost.rhtml")))

(define (show-some some)
  (string-append "show " some))
