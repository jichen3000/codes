class Solution:
    ## TLE 38mins
    def addBoldTag(self, s, words):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
        if not words or not s: 
            return s
        words = set(words)
        q = []
        for ei in range(1, len(s)+1):
            for si in range(ei):
                if s[si:ei] in words:
                    # (si,ei,q).p()
                    while q and q[-1][-1] >= si:
                        si = min(q[-1][0], si)
                        q.pop()
                    q += (si, ei),
                    break
        res = ""
        # q.p()
        pre_ei = 0
        for si, ei in q:
            res += s[pre_ei:si]
            res += "<b>"+s[si:ei]+"</b>"
            pre_ei = ei
        res += s[pre_ei:]
        return res

    def addBoldTag(self, s, words):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
        if not words or not s: 
            return s
        words = set(words)
        q = []
        last_si = len(s)
        for ei in range(len(s), 0, -1):
            for si in range(min(ei, last_si)):
                if s[si:ei] in words:
                    # (si,ei,q).p()
                    if q and q[-1][0] <= ei:
                        q[-1] = (si, q[-1][1])
                    else:
                        q += (si, ei),
                    last_si = si
                    break
        res = ""
        # q.p()
        pre_ei = 0
        for si, ei in reversed(q):
            res += s[pre_ei:si]
            res += "<b>"+s[si:ei]+"</b>"
            pre_ei = ei
        res += s[pre_ei:]
        return res

    def addBoldTag(self, s, words):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
        if not words or not s: 
            return s
        words = set(words)
        q = None
        last_si = len(s)
        ll_si = len(s)
        res = ""
        for ei in range(len(s), 0, -1):
            for si in range(min(ei, last_si)):
                if s[si:ei] in words:
                    # (si,ei,q).p()
                    if q and q[0] <= ei:
                        q = (si, q[1])
                    elif q:
                        res = "<b>"+s[q[0]:q[1]]+"</b>" + s[q[1]:ll_si] + res
                        ll_si = q[0]
                        q = (si, ei)
                    else:
                        q = (si, ei)
                    last_si = si
                    break
        if q:
            res = "<b>"+s[q[0]:q[1]]+"</b>" + s[q[1]:ll_si] + res
            ll_si = q[0]
        res = s[:ll_si] + res
        return res

    def addBoldTag(self, s, words):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
        if not words or not s: 
            return s
        q = []
        res = ""
        for i in range(1, len(s)+1):
            for w in words:
                wn = len(w)
                si = i-wn
                if si >= 0 and w == s[si:i]:
                    while q and q[-1][-1] >= si:
                        si = min(q[-1][0], si)
                        q.pop()
                    q += (si, i),
        # q.p()
        pre_ei = 0
        for si, ei in q:
            res += s[pre_ei:si]
            res += "<b>"+s[si:ei]+"</b>"
            pre_ei = ei
        res += s[pre_ei:]

        return res

    def addBoldTag(self, s, words):
        """
        :type s: str
        :type dict: List[str]
        :rtype: str
        """
        if not words or not s: 
            return s
        q = None
        res = ""
        last_si = len(s)
        for i in range(len(s), 0, -1):
            for w in words:
                si = i-len(w)
                if si >= 0 and w == s[si:i]:
                    # (si, i, q).p()
                    if q and i >= q[0]:
                        q = (min(q[0],si), q[1])
                    else:
                        if q:
                            res = "<b>" + s[q[0]:q[1]] + "</b>" + s[q[1]:last_si] + res
                            last_si = q[0]
                        q = (si, i)
                    # q.p()
        if q:
            res = "<b>" + s[q[0]:q[1]] + "</b>" + s[q[1]:last_si] + res
            last_si = q[0]
        res = s[:last_si] + res
        return res


if __name__ == '__main__':
    from minitest import *

    with test(Solution):
        Solution().addBoldTag("abcxyz123", ["abc","123"]).must_equal("<b>abc</b>xyz<b>123</b>")
        Solution().addBoldTag("aaabbcc",["aaa","aab","bc"]).must_equal("<b>aaabbc</b>c")
        Solution().addBoldTag("aaabbcc", ["d"]).must_equal("aaabbcc")
        Solution().addBoldTag("abcxyz123", ["123"]).must_equal("abcxyz<b>123</b>")
        Solution().addBoldTag("",
                ["QH9sUl","","JfeicLOEtfTBA1D5RFvsley0J2Ut8l","WELTdMnQzTHEaXDfiljZaufDYMXXu2Vk7QUKHNbj0ZTP9NP5dV10zAdv8B7xkLlwOR8yeTxTdAeInR8s1ZDjq8XM77Ru8RRdYEHN2XPorzCMjwngVM0mbKJ8Gq0XVmrw","RFlVw1HmmyVDWilgdFjhnDjOdq6jZAJOn8OrGy61np2yAqPS5328Y4cuHRS7BDTMMGxJBThj8aXqKXBywJlYcldVV4hX67lNUYqZXgbSvR2cdvYrGy","","L2NIjfp8jjHrMqLaAtEJUC7Kc4uxnAc9K8fKF1wNmA31mAYHryp73","1nE6Bb2QOeRS87RbfbHrIpjecaH2cVAmPhNThk8raF7Yn0aM9uDZa0Ij5s3BkpsOgHtYan34bchbeh7i8n3AiyiSPFWgAwzbmQX9rKqPZnCbJL5wc2doAHDQhr5jAGGedLKQdQMH9pRfaKeVUr8Qwhm6WP5HAqd1lqsmzaMyh","t8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLOEtfTBA1D5RFvsley0J2U","nOkqMLkPvsnlzqRkmPp7Bip81gNhLyUhXCCZGEQQHTiRa8KWxKxoLVbBfrzN3PEwd11U","J34MjYDMP1QH9sU","","04Srn4BksGIueQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGH","sKlhjhMajQ","eQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr6BBxFutVShEucJykX12S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLO","","","w5u3pqcyVfoSt6w","","S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl","","ht8m1fH68PxUFl5yJJRi31PA2OVlVpuE5tk4mU8dqP6NfJfeicLOEtfTBA1D5RFvsley0J2Ut8lAaZ","","","KlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr6BBxFutVShEucJykX12S1ni1Oby7anyhm5wGZruXyBqmht8m1fH68PxUFl5yJJRi31PA2OVlVpu","","wiBlClPhKM9uGfbFSqXWELTdMnQzTHEaXDfiljZaufDYMXXu2Vk7QUKHNbj0ZTP9NP5dV10zAdv8B7xkLlwOR8yeTxTdAeInR8s1ZDjq8XM77Ru8RRdYEHN2XPorzCMjwngVM0mbKJ8Gq0XVmrwC7Na3Uvtb2DqU04Srn4Bks","4BksGIueQVsnwdIBnLf5PtSK4jGE82T6T4sk06DIeJtiwsKlhjhMajQOypjSrRo0wEVwO0o0iDwqM7wbe0VvSXaJ3J34MjYDMP1QH9sUla9zFt9EFWQMcwGHik0ANzrPXr3wJlNvmr"]).must_equal(
                "9NLRDk5b4igx8bZGb8wJayqENXuksNK0kIdI6<b></b>")
