...
class TreeNode(object):
    def __init__(self, val, level):
        ...
        self.level = level
    def add(self, val):
        child = TreeNode(val, self.level+1)
        child.parent = self
        self.children += child,
        return child

class Solution(object):
    def largestDivisibleSubset_tree(self, nums):
        """
        :type nums: List[int]
        :rtype: List[int]
        """
        def add_to_node(node, val):
            ...
            for cur_node in node.children:
                if val % cur_node.val == 0:
                    nodes += add_to_node(cur_node,val),
            if len(nodes) == 0:
                return node.add(val)
            deepest_node = nodes[0]
            for cur_node in nodes:
                if cur_node.level > deepest_node.level:
        ...
        for i in xrange(1,len(nums)):
            cur = nums[i]
            cur_node = add_to_node(root,cur)
            if cur_node.level > deepest_node.level:
                deepest_node = cur_node
        results = []
        cur_node = deepest_node
        while cur_node:
            if not (add_1 and cur_node.val == 1):
                results.insert(0,cur_node.val)
