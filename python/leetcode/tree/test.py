...
class Solution(object):
    def fit_in(self,pair, result):
        # (pair, result).p()
        if pair[1] < result[0][0]:
            return 0
            ...
            for i in xrange(len(result)):
                if result[i-1][1] < pair[0] and pair[1] < result[i][0]:
                    return i
                    # result.insert(i, pair)
                    # return len(result)
        if pair in result:
            return -100
        return -1
                
    def puts_in(self,pair, all_results):
        cur_max_len = 0
        # for result in all_results:
        for i in xrange(len(all_results)):
            result = all_results[i]
            insert_index = self.fit_in(pair, result)
            if insert_index >= 0:
                new_result = result[:]
                result.insert(insert_index,pair)
                all_results.append(new_result)
                cur_max_len = max(len(result),cur_max_len)
            if insert_index == -100:
                return 0
        all_results.append([pair])
        return max(cur_max_len,1)
        # if cur_max_len == 0:
        # else:
        #     return cur_max_len
    ...
    def findLongestChain2(self, pairs):
        ...
        max_len = 1
        for pair in pairs:
            cur_len = self.puts_in(pair, all_results)
            # print("cur_len,pair,all_results",cur_len,pair,all_results)
            max_len = max(max_len, cur_len)
        return max_len

    def is_fit(self,pairs):
        if len(pairs) == 2:
            return pairs[0][1] < pairs[1][0]
        else:
            for i in xrange(1,len(pairs)-1):
                if pairs[i-1][1] >= pairs[i][0] or pairs[i][1] >= pairs[i+1][0]:
                    return False
            return True
    ...
    def findLongestChain3(self, pairs):
        ...
        print(len(pairs))
        print(pairs)
        # for i in range(len(pairs), 1, -1):
        #     cur_combinations = combinations(pairs, i)
        #     for cur_pairs in cur_combinations:
        #         if self.is_fit(cur_pairs):
        #             return len(cur_pairs)
        # return 1

    # def fit_(self,pair, result):
    #     # (pair, result).p()
    #     if pair[1] < result[0][0]:
    #         return 0
    #         # result.insert(0,pair)
    ...
    def findLongestChain(self, pairs):
        ...
        cur_chain = [pairs[0]]
        for pair in pairs:
            if cur_chain[-1][1] < pair[0]:
                cur_chain.append(pair)
        return len(cur_chain)

# 646. Maximum Length of Pair Chain

# class Solution(object):
#     def replaceWords(self, the_dict, sentence):
#         """
#         :type the_dict: List[str]
#         :type sentence: str
#         :rtype: str
#         """
#         if len(the_dict) == 0:
#             return sentence
#         the_dict = sorted(the_dict, key=lambda x: len(x))
#         s_list = sentence.split(" ")
#         new_list = []
#         for cur_s in s_list:
#             rooted = False
#             for root in the_dict:
#                 if cur_s.startswith(root):
#                     rooted = True
...
if __name__ == '__main__':
    ...
    # with test("Solution"):
    #     the_dict = ["rat", "batt", "cat", "bat"]
    #     sorted(the_dict, key=len).p()
    #     sentence = "the cattle was rattled by the battery"
    #     Solution().replaceWords(the_dict, sentence).p()
