...
class Solution(object):
    def exist(self, board, word):
        ...
        def get_nexts(i,j,k):
            ...
            for di,dj in dirs:
                ni, nj = i+di, j+dj
                if ni >= 0 and ni < n and nj >=0 and nj < m and not visited[ni][nj] and board[ni][nj] == word[k]:
                    results += (ni, nj),
            return results
        def dfs(i,j,k):
    ...
    def findWords(self, board, words):
        ...
        :rtype: List[str]
        """
        return [w for w in set(words) if self.exist(board, w)]
    ...
    def findWords(self, board, words):
        ...
        def dfs(i, j, cur_word):
            cur_word += board[i][j]
            # (i,j,cur_word, trie.starts_with(cur_word)).p()
            if not trie.starts_with(cur_word):
                return
            if trie.search(cur_word):
                # cur_word.p()
                answers.add(cur_word)
            for di,dj in dirs:
                ni, nj = i+di, j+dj
...
class TrieNodeW(object):
    def __init__(self):
        self.children = defaultdict(TrieNodeW)
        self.word = None

class Solution(object):
    # using trie Optimization
    ...
    def findWords(self, board, words):
        ...
        def dfs(i, j, trie_node):
            c = board[i][j]
            cur_node = trie_node.children[c]
            if cur_node.word:
                answers.append(cur_node.word)
                cur_node.word = None
            board[i][j] = "#"
            if i > 0 and board[i-1][j] in cur_node.children: dfs(i-1, j, cur_node)
            if j > 0 and board[i][j-1] in cur_node.children: dfs(i, j-1, cur_node)
            if i+1 < n and board[i+1][j] in cur_node.children: dfs(i+1, j, cur_node)
            if j+1 < m and board[i][j+1] in cur_node.children: dfs(i, j+1, cur_node)
            board[i][j] = c
...
if __name__ == '__main__':
    ...
    with test(Solution):
        ...
        Solution().findWords(board, words).p()
        # board = [["o","a","a","n"],["e","t","a","e"],["i","h","k","r"],["i","f","l","v"]]
        # words = ["oath","pea","eat","rain"]
        # Solution().findWords(board, words).must_equal(['oath', 'eat'])
