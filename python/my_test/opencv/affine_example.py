...
def __perform_by_normal_line(the_image, bar_value_1, bar_value_2):
    height, width, not_use = the_image.shape
    source_points = numpy.float32( [[0, 0],
                                    [width - 1, 0],
                                    [0, height - 1]])
    target_points = numpy.float32( [[width*0.0, height*0.33],
                                    [width*0.85, height*0.25],
                                    [width*0.15, height*0.7]])
    warp_matrix = cv2.getAffineTransform(source_points, target_points)
    warped_image = cv2.warpAffine(the_image,warp_matrix,(width,height))
    return warped_image
