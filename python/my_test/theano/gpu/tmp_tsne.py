...
def compute_y(P, no_dims, max_iter):
    ...
    def scan_y(cur_step, y_arg, iy_arg, gains_arg, p_arg):
        ...
        indexs_min = (resultb<min_gain).nonzero()
        gains_arg = tensor.set_subtensor(resultb[indexs_min], min_gain)

        # last step in simple version of SNE
        iy_arg = momentum * iy_arg - eta * (gains_arg * dy_arg)
        y_arg = y_arg + iy_arg
        y_arg = y_arg - tensor.tile(tensor.mean(y_arg, 0), (n, 1))
    ...
    results, updates = theano.scan(
            scan_y,
            sequences=steps_sym, 
            outputs_info=[y_sym, iy_sym, gains_sym, p_sym])
    compute_y_fun = theano.function(inputs=[steps_sym, y_sym, iy_sym, gains_sym, p_sym], 
            outputs=results) 

    real_results = compute_y_fun(range(max_iter), Y, iY, gains, P) 
