...
def __calculate_error(dataset_matrix, label_matrix, alphas, b, row_index):
    alpha_labels = multiply(alphas,label_matrix).T
    index_row_to_col = dataset_matrix[row_index,:].T
    fx = float(alpha_labels* (dataset_matrix*index_row_to_col)) + b
    return fx - float(label_matrix[row_index])

def __is_error_in_tolerance(error_value, label_matrix, alphas, constant, tolerance, row_index):
    ...
    return ((label_matrix[row_index]*error_value < -tolerance) and  
                (alphas[row_index] < constant)) or \
                ((label_matrix[row_index]*error_value > tolerance) and
                (alphas[row_index] > 0))

def __cal_edegs(label_matrix, alphas, constant, row_i, row_j):
    ''' Guarantee alphas stay between 0 and constant '''
    if (label_matrix[row_i] != label_matrix[row_j]):
        L = max(0, alphas[row_j] - alphas[row_i])
        H = min(constant, constant + alphas[row_j] - alphas[row_i])
    else:
        L = max(0, alphas[row_j] + alphas[row_i] - constant)
        H = min(constant, alphas[row_j] + alphas[row_i])
    return L,H

def __cal_eta(dataset_matrix, row_i, row_j):
    eta = 2.0 * dataset_matrix[row_i,:]*dataset_matrix[row_j,:].T - \
            dataset_matrix[row_i,:]*dataset_matrix[row_i,:].T - \
            dataset_matrix[row_j,:]*dataset_matrix[row_j,:].T
    return eta

def __is_moving_enough(new_value, old_value):
    return abs(new_value - old_value) > 0.00001 

def __cal_b(error_i, error_j, b, alphas, dataset_matrix, label_matrix, constant,
    ...
    b1 = b - error_i- label_matrix[row_i]*(alphas[row_i]-alpha_i_old)*\
            ...
            dataset_matrix[row_i,:]*dataset_matrix[row_j,:].T
    b2 = b - error_j- label_matrix[row_i]*(alphas[row_i]-alpha_i_old)*\
            dataset_matrix[row_i,:]*dataset_matrix[row_j,:].T - \
            label_matrix[row_j]*(alphas[row_j]-alpha_j_old)*\
            dataset_matrix[row_j,:]*dataset_matrix[row_j,:].T
    if (0 < alphas[row_i]) and (constant > alphas[row_i]): b = b1
    elif (0 < alphas[row_j]) and (constant > alphas[row_j]): b = b2
    else: b = (b1 + b2)/2.0
    return b

def smo_once(b, alphas, dataset_matrix, label_matrix, constant, tolerance, row_count):

    ''' main '''    
    alpha_pairs_changed = 0
    for i in range(row_count):
        error_i = __calculate_error(dataset_matrix, label_matrix, alphas, b, i)
        # enter optimization if alphas can be changed.
        if not __is_error_in_tolerance(error_i, label_matrix, alphas, constant, tolerance, i):
            continue
        # random select second alpha
        j = romdom_select_j(i,row_count)
        error_j = __calculate_error(dataset_matrix, label_matrix, alphas, b, j)
        alpha_i_old = alphas[i].copy();
        alpha_j_old = alphas[j].copy();
        L, H = __cal_edegs(label_matrix, alphas, constant, i,j)
        # Guarantee alphas stay between 0 and constant
        if L==H: 
            continue
        # Eta is the optimal amount to change alpha[j].
        eta = __cal_eta(dataset_matrix, i, j)
        if eta >= 0: 
            # print "eta>=0"
            continue
        ...
        b = __cal_b(error_i, error_j, b, alphas, dataset_matrix, label_matrix, constant,
            i, j, alpha_i_old, alpha_j_old)
        alpha_pairs_changed += 1
    return alpha_pairs_changed, b, alphas, dataset_matrix, label_matrix, constant, tolerance, row_count 


def smo_simple(dataset, labels, constant, tolerance, max_count):
    dataset_matrix = mat(dataset)
    label_matrix = mat(labels).T
    b = 0; row_count,n = shape(dataset_matrix)
    alphas = mat(zeros((row_count,1)))
    results = continue_call_until_no_change(smo_once, max_count,
        b, alphas, dataset_matrix, label_matrix, constant, tolerance, row_count)
    return results[1:3]
...
if __name__ == '__main__':
    ...
    with test("calculate_error"):
        small_dataset_matrix = mat(small_dataset)
        small_label_matrix = mat(small_labels).T
        m,n = shape(small_dataset_matrix)
        b=0
        small_alphas = mat(zeros((m,1)))
        row_index = 1
        __calculate_error(small_dataset_matrix, small_label_matrix, 
