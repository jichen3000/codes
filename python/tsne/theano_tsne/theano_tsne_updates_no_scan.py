...
def compute_y(P, no_dims, max_iter):
    ...
    # p_arg = theano.shared(cuda.gpu_from_host(P.astype(FLOATX)))
    ...
    # dy_arg = [tensor.cast(tensor.sum(tensor.tile(PQ[:,i] * num[:,i], (no_dims, 1)).T * (y_arg[i,:] - y_arg), 0),FLOATX) for i in range(n)]
    # dy_arg = tensor.matrix(dy_arg, dtype=FLOATX)
    # dy_arg = theano.tensor.stack(dy_arg)
    # dy_arg = theano.printing.Print('this is a very important value')(dy_arg.shape)
    ...
    # y_arg.set_value(new_y_arg)
    # iy_arg.set_value(new_iy_arg)
    # gains_arg.set_value(new_gains_arg)
    # p_arg.set_value(new_p_arg)
    ...
    # theano.printing.pydotprint(compute_y_fun, 
    #         var_with_name_simple=True, 
    #         compact=True, 
    #         outfile='data/compute_y_fun.png', format='png')
    # theano.printing.debugprint(compute_y_fun)
