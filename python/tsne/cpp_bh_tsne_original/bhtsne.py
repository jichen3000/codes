...
def run_bh_tsne(data, no_dims=2, perplexity=50, theta=0.5, randseed=-1, verbose=False):
    '''
    ...
    # print("dd:",data.shape)
    res = []
    for result in bh_tsne(data, no_dims=no_dims, perplexity=perplexity, theta=theta, randseed=randseed,verbose=verbose):
        sample_res = []
        for r in result:
            sample_res.append(r)
...
def handle_data():
    ...
    Y = run_bh_tsne(data, no_dims=no_dims, perplexity=perplexity, theta=theta,
