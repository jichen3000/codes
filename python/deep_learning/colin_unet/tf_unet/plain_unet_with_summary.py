...
def one_conv2d(cur_layer_name, the_input, weight_shape, bias_shape, stddev, keep_prob, is_summary=False):
    with tf.name_scope(cur_layer_name):
        with tf.name_scope('conv2d'):
            with tf.name_scope('weights'):
                cur_w = tf.Variable(tf.truncated_normal(weight_shape, stddev))
                if is_summary:
                    variable_summaries(cur_w)
            conv = tf.nn.conv2d(the_input, cur_w, strides=[1, 1, 1, 1], padding='VALID')
        ...
        with tf.name_scope('relu'):
            with tf.name_scope('biases'):
                cur_b = tf.Variable(tf.constant(0.1, shape=bias_shape))
                if is_summary:
                    variable_summaries(cur_b)
            tmp_h_conv = tf.nn.relu(conv_dropout + cur_b)
    return tmp_h_conv, cur_w, cur_b

def two_conv2d(layer_name, input_x, weight1_shape, weight2_shape, bias_shape, stddev, 
        keep_prob, is_max_pool, pool_size, is_summary=False):
    with tf.name_scope(layer_name):
        conv1, w1, b1 = one_conv2d(layer_name+"_first", input_x, weight1_shape, 
                bias_shape, stddev, keep_prob, is_summary)
        conv2, w2, b2 = one_conv2d(layer_name+"_second", conv1, weight2_shape,
                bias_shape, stddev, keep_prob, is_summary)
        max_pool_conv = conv2
        if is_max_pool:
            with tf.name_scope('max_pool'):
                max_pool_conv = tf.nn.max_pool(conv2, 
                        ksize=[1, pool_size, pool_size, 1], strides=[1, pool_size, pool_size, 1], 
                        padding='VALID')

    return max_pool_conv, conv2, w2, b2, w1, b1 


def two_deconv2d(layer_name, input_x, dw_h_conv, pool_size, features, filter_size, stddev, keep_prob, is_summary=False):
    with tf.name_scope(layer_name):
        with tf.name_scope(layer_name+"_concat"):
            x_shape = tf.shape(input_x)
            ...
            with tf.name_scope('conv2d_transpose'):
                ...
                h_deconv = tf.nn.conv2d_transpose(input_x, wd, output_shape, strides=[1, pool_size, pool_size, 1], padding='VALID')
            with tf.name_scope('relu'):
                with tf.name_scope('biases'):
                    bd = tf.Variable(tf.constant(0.1, shape=[features//2]))
                    if is_summary:
                        variable_summaries(bd)
                h_deconv_relu = tf.nn.relu(h_deconv + bd)
            h_deconv_concat = crop_and_concat(dw_h_conv, h_deconv_relu)

        weight1_shape = [filter_size, filter_size, features, features//2]
        weight2_shape = [filter_size, filter_size, features//2, features//2]
        bias_shape = [features//2]
        conv1, w1, b1 = one_conv2d(layer_name+"_first", h_deconv_concat, weight1_shape, 
                bias_shape, stddev, keep_prob, is_summary)
        conv2, w2, b2 = one_conv2d(layer_name+"_second", conv1, weight2_shape, 
                bias_shape, stddev, keep_prob, is_summary)
        
    return conv2, w2, b2, w1, b1
...
def create_unet(channels=3, n_class=2, layers=3, features_root=16, **kwargs):
    ...
    for layer in range(0, layers):
        ...
        layer_name = "down_{}".format(layer)
        if layer == 0:
            weight1_shape = [filter_size, filter_size, channels, features]
        else:
            weight1_shape = [filter_size, filter_size, features//2, features]
        weight2_shape = [filter_size, filter_size, features, features]
        bias_shape = [features]
        is_max_pool = layer < layers-1
        in_node, dw_h_convs[layer], w2, b2, w1, b1 = two_conv2d(layer_name, in_node, 
                weight1_shape, weight2_shape, bias_shape, stddev, keep_prob, 
                is_max_pool, pool_size, is_summary)
    ...
    for layer in range(layers-2, -1, -1):
        ...
        in_node, w2, b2, w1, b1 = two_deconv2d(layer_name, in_node, 
                dw_h_conv, pool_size, features, filter_size, stddev, 
                keep_prob, is_summary)
    ...
    # Output Map
    layer_name = "output_map"
    weight_shape = [1, 1, features_root, n_class]
    bias_shape = [n_class]
    output_map, weight, bias = one_conv2d("output_map", in_node, weight_shape, 
            bias_shape, stddev, is_summary)
...
def toy_problem():
    ...
    unet_hash = create_unet(channels=generator.channels, n_class=generator.n_class, 
        layers=3, features_root=16, is_summary=True)
...
toy_problem()
print("tensorboard --logdir=unet_trained/")
