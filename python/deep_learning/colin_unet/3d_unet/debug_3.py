...
from unet_3d import create_3d_unet, create_3d_unet_01,create_3d_unet_02
...
def cal_class_weights(the_labels):
    ...
    class_weightsruby = class_weights.astype(np.float32)
...
def brain():
    ...
    n_class = 2
    flat_logits = tf.reshape(pred, [-1, n_class])
    ...
    #         loss,global_step=global_step)
    optimizer = tf.train.AdadeltaOptimizer().minimize(
            loss,global_step=global_step)
