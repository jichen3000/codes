...
def build_3d_unet(input_shape, class_count, layer_count=3, features_root=16):    
    ...
    for layer in range(layer_count-2, -1, -1):
        ...
        output_shape = [None]+[x.value for x in down_layers[layer].shape[1:4]] + [features * 4]
        cur_layer = Conv3DTranspose(features * 4, kernel_size, output_shape=output_shape,
...
def build_3d_unet_for_theano(input_shape, class_count, layer_count=3, features_root=16):    
    # class_count = 2
    # layer_count = 3
    # features_root=16
    # input_shape = (128, 128, 128, 1)
    ...
    for layer in range(layer_count-2, -1, -1):
        ...
        if layer == 0:
            output_shape = [None, 128, 128, 128, 64]
        else:
            output_shape = [None, 64, 64, 64, 128]
        cur_layer = Conv3DTranspose(features * 4, kernel_size, output_shape=output_shape,
                strides=pool_size, padding='same', activation='relu')(cur_layer)
        cur_layer = concatenate([down_layers[layer], cur_layer])
    ...
    # cur_layer = Reshape((-1, class_count))(cur_layer)
    transpose(4,0,1,2,3)
    flatten or reshape
    cur_layer = Activation(K.softmax)(cur_layer)
    the_net["train"] = cur_layer
